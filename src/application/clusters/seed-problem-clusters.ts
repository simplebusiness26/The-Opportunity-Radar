 ...
 * into a problem. This function keeps that rule: it first groups similar loose
 * evidence provisionally, then persists only groups that already satisfy the
 * normal cluster-readiness gate (three distinct evidence units and two
 * independent origins by default). Nothing is created from a singleton.
 *
 * Without a semantic embedding provider the same lexical-v1 matching used by
 * the rest of Radar is used here. That limitation is recorded rather than
 * hidden; connecting embeddings later improves recall, not the safety gate.
 */
export async function seedProblemClusters(
  ...
  const ordered = rows
    ...
    .sort((a, b) => b.evidence.strength - a.evidence.strength || b.evidence.lastSeenAt.getTime() - a.evidence.lastSeenAt.getTime());
